...
import type { TeacherClass, Student, ClassNotice, Activity, ActivitySubmission, Module, User, AttendanceSession, AttendanceRecord, AttendanceStatus, Turno, ClassInvitation } from '../types';
...
type ClassCardTab = 'overview' | 'notices' | 'modules' | 'activities' | 'students';

const StudentListItem: React.FC<{ student: Student }> = React.memo(({ student }) => (
    <div className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg transition-colors duration-200">
        <p className="font-semibold text-slate-800 dark:text-slate-200 hc-text-primary">{student.name}</p>
    </div>
));
...
const ModuleListItem: React.FC<{ module: Module; onEdit: (module: Module) => void; onDelete: () => void; isOwner: boolean; }> = React.memo(({ module, onEdit, onDelete, isOwner }) => (
    <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50 flex justify-between items-center">
        <div>
            <p className="font-semibold text-slate-800 dark:text-slate-200">{module.title}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">
                {module.series} • {module.materia}
            </p>
        </div>
            ...
            {isOwner ? (
                <>
                    <button
                        ...
                        className="px-4 py-2 text-sm font-semibold text-indigo-600 bg-indigo-100 rounded-lg hover:bg-indigo-200 dark:bg-indigo-500/20 dark:text-indigo-300 dark:hover:bg-indigo-500/30 hc-button-override"
                    >
                        Editar
                    </button>
                    <button
                        onClick={onDelete}
                        className="px-4 py-2 text-sm font-semibold text-red-600 bg-red-100 rounded-lg hover:bg-red-200 dark:bg-red-500/20 dark:text-red-300 dark:hover:bg-red-500/30 hc-button-override"
                    >
                        Excluir
                    </button>
                </>
            ) : (
                <span className="px-4 text-xs text-slate-500 dark:text-slate-400 italic">Somente leitura</span>
            )}
        </div>
    </div>
));
...
interface ClassCardProps {
    ...
    modules: Module[]; // New prop: Filtered list of modules for this class
...
const ClassCard: React.FC<ClassCardProps> = React.memo(({ classData, modules, onPostNoticeClick, onViewSubmissionsClick, onDeleteModule, user, onFetchClassDetails }) => {
    ...
    // Safe access to students array to prevent crashes
    ...
    const handleTabChange = (tab: ClassCardTab) => {
        ...
        if ((tab === 'activities' || tab === 'students') && !classData.isFullyLoaded) {
            setLoadingDetails(true);
            onFetchClassDetails(classData.id);
        }
        // Modules are now passed as props, so no need to lazy load them here specifically for the list
    };
    ...
    const safeModules = modules || []; // Use the prop based modules
    ...
    const tabs: { id: ClassCardTab; label: string; count: number; icon?: React.ReactNode }[] = [
        ...
        { id: 'modules', label: 'Módulos', count: safeModules.length },
        { id: 'activities', label: 'Atividades', count: classData.activityCount ?? safeActivities.length },
        { id: 'students', label: 'Alunos', count: studentCount },
    ];
    
    const renderTabContent = () => {
        if (loadingDetails && activeTab !== 'overview' && activeTab !== 'notices' && activeTab !== 'modules') {
        ...
        switch (activeTab) {
            ...
            case 'modules':
                return safeModules.length > 0 ? (
                    <div className="space-y-4">
                         {safeModules.map(module => <ModuleListItem 
                            key={module.id} 
                            module={module} 
                            onEdit={startEditingModule} 
                            onDelete={() => {
                                if (window.confirm(`Tem certeza que deseja apagar o módulo "${module.title}"?`)) {
                                    onDeleteModule(classData.id, module.id);
                                }
                            }}
                            isOwner={!!user && (user.id === module.creatorId || user.role === 'admin')}
                        />)}
                    </div>
            ...
            case 'students':
                return students.length > 0 ? (
                    <div className="divide-y divide-slate-200 dark:divide-slate-700 -m-6 hc-border-override">
                       {students
                        .slice()
                        .sort((a, b) => a.name.localeCompare(b.name))
                        .map(student => <StudentListItem key={student.id} student={student} />)}
                    </div>
                ) : <p className="text-center text-sm text-slate-500 py-8">Nenhum aluno nesta turma.</p>;
            default: return null;
        }
    };
...
const TeacherDashboard: React.FC = () => {
    const { teacherClasses, modules, handlePostNotice, handleCreateClass, handleDeleteModule, handleGradeActivity, fetchData, isLoading, fetchClassDetails, fetchModulesLibrary, pendingInvitations, handleAcceptInvite, handleDeclineInvite, isSubmitting } = useTeacherData();
    const { user } = useAuth();
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 hc-text-primary">Minhas Turmas</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 hc-text-secondary">Gerencie suas turmas e acompanhe o progresso dos alunos</p>
                </div>
            ...
            <div className="space-y-6">
                {teacherClasses.map(c => {
                    // DATA STRATEGY: Filter global modules for this specific class ID
                    // This ensures modules appear without needing nested data in the class object
                    const classModules = modules.filter(m => m.classIds?.includes(c.id));
