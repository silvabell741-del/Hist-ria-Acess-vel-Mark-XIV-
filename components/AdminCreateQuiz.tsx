...
const MultiSelect: React.FC<{
    ...
    error?: boolean;
    helperText?: string;
}> = ({ options, selected, onChange, label, error, helperText }) => {
    ...
    return (
        <div className="space-y-2">
            <label className={`block text-sm font-medium ${error ? 'text-red-600 dark:text-red-400' : 'text-gray-700 dark:text-slate-300'}`}>
                {label} <span className="text-red-500">*</span>
            </label>
            <div className={`p-3 border rounded-md bg-white dark:bg-slate-700 max-h-48 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 ${error ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-slate-600'}`}>
                {options.map(option => (
            ...
            {(error || helperText) && (
                <p className={`text-xs ${error ? 'text-red-500 font-semibold' : 'text-slate-500 dark:text-slate-400'}`}>
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                 ...
                 <div className="space-y-6">
                    <div className="space-y-2">
                        <label className={`block text-sm font-medium ${formErrors.title ? 'text-red-600 dark:text-red-400' : 'text-gray-700 dark:text-slate-300'}`}>
                        ...
                        />
                        {formErrors.title && <p className="text-xs text-red-500">O título é obrigatório.</p>}
                    </div>
                    <div className="space-y-2">
                ...
                {questions.map((q, idx) => (
                    <Card key={q.id}>
                        ...
                        <div className="space-y-4">
                            ...
                            />
                             <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Alternativas (Marque a correta)</label>
                                ...
                                <button onClick={() => addChoice(q.id)} className="text-sm text-indigo-600 hover:underline">+ Adicionar Alternativa</button>
                             </div>
                        </div>
                    </Card>
                ))}
